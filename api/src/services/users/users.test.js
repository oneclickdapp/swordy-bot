...
describe('users', () => {
  ...
  scenario('creates a user', async (scenario) => {
    const result = await createUser({
      input: {
        address: 'String1032804',
        discordHandle: 'String5086683',
        authDetailId: 'scenario.user.two.authDetailId',
      },
    })

    expect(result.address).toEqual('String1032804')
    expect(result.discordHandle).toEqual('String5086683')
  ...
  scenario('updates a user', async (scenario) => {
    ...
    const result = await updateUser({
      id: original.id,
      input: { address: 'String26797532' },
    })

    expect(result.address).toEqual('String26797532')
  })
